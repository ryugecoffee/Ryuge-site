/**
 * 送料計算モジュール（CommonJS - server/index.js 用）
 * src/lib/shipping.js（ESM）と同一ロジックを維持すること。
 */

const PACKAGING_WEIGHT_G = 150;
const MAX_WEIGHT_G = 2000;
...
function calcItemsWeightG(items) {
...
function calcShipping(items, countryCode) {
  ...
  if (!row) return { error: "OVER_WEIGHT" };
